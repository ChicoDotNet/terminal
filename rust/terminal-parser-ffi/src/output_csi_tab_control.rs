...
pub enum OutputCsiTabControlKind {
    None = 0,
    Clear = 1,
    Set = 2,
}
...
#[repr(C)]
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct OutputCsiTabControlPlan {
    pub kind: u32,
    pub value: i32,
}

impl Default for OutputCsiTabControlPlan {
    fn default() -> Self {
        Self {
            kind: OutputCsiTabControlKind::None as u32,
            value: 0,
        }
    }
}

fn tab_control_plan(action: OutputAction) -> Option<OutputCsiTabControlPlan> {
    let (kind, value) = match action {
        OutputAction::TabClear(value) => (OutputCsiTabControlKind::Clear, value),
        OutputAction::TabSet(value) => (OutputCsiTabControlKind::Set, value),
        _ => return None,
    };
    Some(OutputCsiTabControlPlan {
        kind: kind as u32,
        value,
    })
}
...
struct BatchPlanDispatch {
    plans: Vec<OutputCsiTabControlPlan>,
}

impl TermDispatch for BatchPlanDispatch {
    fn dispatch(&mut self, action: OutputAction) {
        if let Some(plan) = tab_control_plan(action) {
...
/// Replays the complete CSI tab-clear/tab-set parameter list through the Rust
/// output engine. Inputs and outputs remain caller-owned.
#[unsafe(no_mangle)]
pub extern "C" fn terminal_parser_ffi_output_csi_tab_control_plans(
    identifier: u64,
    values: *const i32,
    value_count: usize,
    out_plans: *mut OutputCsiTabControlPlan,
...
mod tests {
    use super::{
        OutputCsiTabControlKind, OutputCsiTabControlPlan,
        terminal_parser_ffi_output_csi_tab_control_plans,
    };
    use crate::FfiStatus;
    use terminal_parser::state_machine::{MAX_PARAMETER_COUNT, VtId};

    fn batch(id: &str, values: &[i32]) -> Vec<OutputCsiTabControlPlan> {
        ...
        let mut plans = vec![OutputCsiTabControlPlan::default(); required];
        assert_eq!(
            terminal_parser_ffi_output_csi_tab_control_plans(
                VtId::from_ascii(id).value(),
                values.as_ptr(),
    ...
    #[test]
    fn csi_tab_control_batch_replays_clear_and_set_contracts() {
        for (id, expected_kind) in [
            ("g", OutputCsiTabControlKind::Clear),
            ("?W", OutputCsiTabControlKind::Set),
        ] {
            let plans = batch(id, &[0, 1, 3]);
            assert_eq!(plans.len(), 3, "id={id:?}");
            assert_eq!(plans.iter().map(|plan| plan.value).collect::<Vec<_>>(), [0, 1, 3]);
            assert!(plans.iter().all(|plan| plan.kind == expected_kind as u32));
        }
    }

    #[test]
    fn csi_tab_control_batch_preserves_empty_parameter_default_and_unrelated_csi() {
        let plans = batch("g", &[]);
        assert_eq!(plans.len(), 1);
        assert_eq!(plans[0].kind, OutputCsiTabControlKind::Clear as u32);
    ...
    #[test]
    fn csi_tab_control_batch_reports_capacity_and_validates_arguments() {
        let values = [0, 1];
        let mut required = 0usize;
        let mut one = OutputCsiTabControlPlan::default();
        assert_eq!(
            terminal_parser_ffi_output_csi_tab_control_plans(
                VtId::from_ascii("g").value(),
                values.as_ptr(),
                values.len(),
                &mut one,
                1,
